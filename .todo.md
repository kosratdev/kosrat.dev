...
- [ ] **Extract Slug Utilities**: Create `src/utils/slug-utils.ts` to centralize slug cleaning logic
  - [ ] Move `cleanSlug()` function from multiple files to shared utility
  - [ ] Create `extractSlugParts()` function for consistent slug parsing
  - [ ] Update all files using slug manipulation to use centralized functions

- [ ] **Refactor Content Utils**: Split oversized `src/utils/content-utils.ts` (380+ lines)
  - [ ] Create `src/utils/content/post-utils.ts`
  - [ ] Create `src/utils/content/course-utils.ts`
  - [ ] Create `src/utils/content/archive-utils.ts`
  - [ ] Create `src/utils/content/types.ts` for shared types
